...
The assessment environment was prepared by deploying OWASP Juice Shop within a Docker container and configuring Burp Suite Professional as the primary interception platform.
...
Initial reconnaissance focused on identifying the application's exposed functionality and attack surface.

Activities performed:

- Browse application functionality
- Build Burp Target Site Map
...
Authentication testing evaluated the application's login workflow and session handling mechanisms.
...
Authorization testing focused on evaluating resource access controls and authenticated user permissions.
...
- Administrative endpoint assessment
...
Application endpoint discovery was performed to identify publicly accessible resources exposed through the application's REST API.
...
- Administrative endpoint identification
...
Critical application endpoints were manually manipulated using Burp Repeater to evaluate server-side validation and business logic enforcement.
...
Application functionality was manually validated to verify expected operational behavior.
